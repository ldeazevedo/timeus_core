...
public final class ScriptManager implements Runnable
{
	...
	private final void load()
	{
		...
		ThreadPoolManager.getInstance().scheduleGeneralAtFixedRate(this, 0, PERIOD);
	}
	...
	public void run()
	{
		...
		for (ScheduledQuest script : _scheduled)
		{
			...
			if (eta > 0)
				ThreadPoolManager.getInstance().scheduleGeneral(new Scheduler(script), PERIOD - eta);
		}
	}
	...
	private final class Scheduler implements Runnable
	{
		...
		public void run()
		{
			...
			if (eta > 0)
				ThreadPoolManager.getInstance().scheduleGeneral(this, PERIOD - eta);
		}
	}
