	...
	protected void runImpl()
	{
		...
		switch (_actionId)
		{
			...
			case 21: // Change Movement Mode (pet follow/stop)
				...
				if (pet.getAI().getIntention() == CtrlIntention.ATTACK)
			...
			case 22: // Attack (pet attack)
				if (!(target instanceof L2Character) || pet == null || pet == target || activeChar == target || pet.isAttackingDisabled())
					return;
				...
				if (target.isAutoAttackable(activeChar) || _ctrlPressed || target instanceof L2NpcInstance)
				{
					...
					else if (pet.getNpcId() != L2SiegeSummonInstance.SIEGE_GOLEM_ID)
					{
						...
						else if (pet.getAttackEndTime() > System.currentTimeMillis())
							return;
