...
import net.sf.l2j.gameserver.model.entity.events.L2Event;
...
public final class L2PcInstance extends L2Playable
{
	...
	public void setClassId(int Id)
	{
		...
		try
		{
			...
			if (Config.AUTO_LEARN_SKILLS || Config.AUTO_LEARN_SKILLS_DONATORS.contains(getName()))
				rewardSkills();
		}
		finally
		{
	...
	public void standUp()
	{
		...
		else if (_isSitting && !isInStoreMode() && !isAlikeDead() && !isParalyzed())
		{
	...
	public void onKillUpdatePvPKarma(L2Playable target)
	{
		...
		if (atEvent && targetPlayer.atEvent)
			return;
		...
		for (L2PcInstance player : L2World.getInstance().getPlayers())
			if (player.showPVP)
				if (target instanceof L2PcInstance)
					if (getKarma() <= 0)
						player.sendPacket(new CreatureSay(0, 8, "[Pvp]", getName() + ": ha matado a: " + target.getName()));
					else if (getKarma() > 0)
						player.sendPacket(new CreatureSay(0, 8, "[PK]", getName() + ": ha matado a: " + target.getName()));
	}
	...
	private static boolean isInEvent(L2PcInstance pc)
	{
	...
	{
		for (String name : Config.FREE_BOX_CHARS)
		{
		...
		if (getClient() != null && getClient().getConnection() != null && !getClient().getConnection().isClosed() && getClient().getConnection().getInetAddress() != null)
		{
			...
			final Collection<L2PcInstance> allPlayers = L2World.getInstance().getPlayers();
			for (L2PcInstance player : allPlayers)
				if (player != null)
				{
					if (!player.isOnline() || player.getClient() == null || player.getClient().getConnection() == null)
						continue;
					else if (player.getClient().getConnection().isClosed() || player.getClient().getConnection().getInetAddress() == null)
						continue;
					else if (player.getName().equals(getName()))
						continue;
	...
	public boolean showPVP = false;
	
	public void showPvP()
	{
		showPVP = !showPVP ? true : false;
		sendMessage(showPVP ? "Activaste los anuncios de pvp" : "Desactivaste los anuncios de pvp");
		return;
	}
	
	public boolean isChatGlobal = true;
	...
	public void setChatGlobal(boolean a)
	{
		isChatGlobal = a;
	}
}