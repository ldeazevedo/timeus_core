...
public class TvTManager
{
	...
	public void startReg()
	{
		...
		{
			Broadcast.announceToOnlinePlayers("TvT Event: Evento cancelado.");
		...
		{
			Broadcast.announceToOnlinePlayers("TvT Event (" + (TvTEvent.isLow ? "65-" : "65+") + "): Abierto por " + Config.TVT_EVENT_PARTICIPATION_TIME + " minuto(s).");
			Broadcast.announceToOnlinePlayers("Para registrarte escribi .register");
	...
	public void startEvent()
	{
		...
		{
			Broadcast.announceToOnlinePlayers("TvT Event: Evento cancelado por falta de participantes.");
		...
		{
			TvTEvent.sysMsgToAllParticipants("TvT Event: Enviando todos a la arena en " + Config.TVT_EVENT_START_LEAVE_TELEPORT_DELAY + " segundo(s).");
	...
	public void endEvent()
	{
		...
		TvTEvent.sysMsgToAllParticipants("TvT Event: Enviando todos a Giran en: " + Config.TVT_EVENT_START_LEAVE_TELEPORT_DELAY + " segundo(s).");
		TvTEvent.stopFight();
	...
	class TvTStartTask implements Runnable
	{
		...
		private void announce(long time)
		{
			...
			else if (time >= 60)
			{
				if (TvTEvent.isParticipating())
				{
					Broadcast.announceToOnlinePlayers("TvT Event: " + (time / 60) + " minuto(s) hasta que cierre el registro!");
				}
				else if (TvTEvent.isStarted())
				{
					TvTEvent.sysMsgToAllParticipants("TvT Event: " + (time / 60) + " minuto(s) hasta que el evento finalize!");
				}
			}
			else
			{
				if (TvTEvent.isParticipating())
				{
					Broadcast.announceToOnlinePlayers("TvT Event: " + time + " segundo(s) hasta que cierre el registro!");
				}
				else if (TvTEvent.isStarted())
				{
					TvTEvent.sysMsgToAllParticipants("TvT Event: " + time + " segundo(s) hasta que el evento finalize!");
				}
			}
		}
	}
